class User < ActiveRecord::Base
  ...
  def self.top_rated
    self.select('users.*') # Select all attributes of the user
        ...
        .joins(:posts) # Ties the posts table tot he users table, via the user_id
        .joins(:comments) # Ties the comments table to the users table, via the user_id
        .group('users.id') # Instructs the database to the group the results so that each user will be returned in the distinct row
